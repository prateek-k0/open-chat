  ...
  return (
    <div className="flex flex-col h-full max-h-full">
      <div className="header bg-slate-900 border-b border-slate-700 text-white text-xl px-8 items-center h-16 flex justify-between font-mono">
        {room.isDM === true ? <FetchUserName userId={otherUsers[0]} /> : room.roomName} 
      ...
      <div className="textbox border-t border-slate-700 text-white text-lg px-8 py-4 h-15 flex gap-4">
        <input onKeyUp={keyUpHandler} className="flex-1 px-4 text-sm py-2 rounded-md text-black outline-none" type="text" onChange={(e) => setMessage(e.target.value)} value={message} placeholder="Enter message" />
        <button onClick={sendMessageHandler} className="rounded-md text-sm border border-white px-4 py-1 hover:bg-slate-600 bg-slate-900">Send</button>
      </div>
    </div>
  )
}
